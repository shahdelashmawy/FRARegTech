...
def upgrade() -> None:
    ...
    # If tables already exist (created by init_db before alembic ran), skip DDL
    ...
    try:
        ...
        log.warning(f"pgvector extension not available, skipping vector columns: {e}")
        conn.execute(sa.text("ROLLBACK TO SAVEPOINT pgvector_check"))

    if tables_exist:
        log.info("Tables already exist (created by init_db) — skipping CREATE TABLE statements")
    else:
        # Create users table
        op.create_table(
            "users",
    ...
    if pgvector_available:
        try:
            ...
            log.warning(f"Could not set vector column types: {e}")
